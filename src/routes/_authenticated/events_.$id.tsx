...
import { StatusBadge, eventTone, paymentTone, rsvpTone, taskTone } from "@/components/console/status-badge";
...
function EventDetail() {
    ...
    const events = useCollection("events", { filters: [{ column: "id", op: "eq", value: id }] }, `event-${id}`);
    ...
    const tasks = useCollection("tasks", { filters: [{ column: "event_id", op: "eq", value: id }] }, `tasks-${id}`);
    ...
    const upcoming = useMemo(() => {
        ...
        return [...fromCalendar, ...fromPayments].sort((a, b) => a.date.localeCompare(b.date)).slice(0, 5);
    ...
    const itemColumns: ReadonlyArray<Column<Row<"event_budget_items">>> = [
        { key: "label", header: "Line item", cell: (row) => row.label },
        {
            key: "kind",
            header: "Type",
            cell: (row) => <StatusBadge tone={row.kind === "income" ? "success" : "neutral"}>{row.kind}</StatusBadge>,
        },
        {
            key: "category",
            header: "Category",
            cell: (row) => (categories.data ?? []).find((category) => category.id === row.category_id)?.name ?? "-",
        },
        {
            key: "planned",
            header: "Planned",
    ...
    const vendorColumns: ReadonlyArray<Column<Row<"event_vendors">>> = [
        ...
        },
        {
            key: "status",
            header: "Status",
            cell: (row) => <StatusBadge tone={paymentTone[row.status] ?? "neutral"}>{row.status}</StatusBadge>,
        },
        {
            key: "actions",
            header: "",
    ...
    const taskColumns: ReadonlyArray<Column<Row<"tasks">>> = [
        ...
        { key: "due", header: "Due", cell: (row) => formatDate(row.due_date) },
        {
            key: "status",
            header: "Status",
            cell: (row) => <StatusBadge tone={taskTone[row.status] ?? "neutral"}>{row.status}</StatusBadge>,
        },
    ];
    ...
    return (
        <>
            ...
            <StatGrid>
                ...
                />
                <StatCell label="Actual income" value={formatMoney(finance.actualIncome)} caption="Linked receipts" />
                <StatCell
                    label="Net position"
            ...
            <Tabs defaultValue="overview" className="mt-6">
                                    ...
                                    <table className="w-full text-sm">
                                        <thead>
                                            <tr className="border-b border-stroke">
                                                ...
                                                <th className="label-mono px-6 py-3 text-right font-normal">
                                                    Variance
                                                </th>
                                            </tr>
                                        </thead>
                ...
                <TabsContent value="budget" className="mt-4">
                    <Panel>
                        ...
                        />
                        <DataTable columns={itemColumns} rows={items.data ?? []} emptyLabel="No budget lines yet" />
                    </Panel>
                </TabsContent>
                ...
                <TabsContent value="vendors" className="mt-4">
                    <Panel>
                        ...
                        />
                        <DataTable columns={vendorColumns} rows={vendors.data ?? []} emptyLabel="No vendors yet" />
                    </Panel>
                </TabsContent>
            ...
            <RecordDialog
                ...
                fields={guestFields}
                initial={dialog?.kind === "guest" ? (dialog.editing ?? { rsvp: "invited", party_size: "1" }) : null}
            ...
            <RecordDialog
                ...
                onSubmit={(values) => {
                    if (dialog?.kind === "payment" && dialog.editing) {
                        updatePayment.mutate({ id: dialog.editing.id, values: paymentPayload(values) } as never, {
                            onSuccess: () => setDialog(null),
                        });
                    } else {
            ...
            <RecordDialog
                ...
                title={dialog?.kind === "calendar" && dialog.editing ? "Edit calendar item" : "Add calendar item"}

...
function UsersPage() {
  ...
  const adminCount = rows.filter((profile) =>
    (rolesByUser.get(profile.id) ?? []).includes("administrator"),
  ).length;
    ...
    <>
      ...
      <Panel className="mt-6">
        ...
        <PanelBody className="space-y-3 text-sm text-grey">
          <p>
            Roles live in a dedicated table, never on the profile, so a compromised profile update
            can never grant administrator rights. Administrators can promote or demote members from
            this page; an administrator can never remove their own admin role, so the workspace can
            never lock itself out.
          </p>
          <p>
            Every table enforces row-level security scoped to your user id. Reads and writes are
            checked by the database itself, not by the interface.
          </p>
        </PanelBody>
      </Panel>
    </>

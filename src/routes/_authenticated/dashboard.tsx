...
function Dashboard() {
  ...
  const todayEntries = useMemo(() => {
    ...
    const fromTasks = (tasks.data ?? [])
      .filter(
        (task) =>
          task.status !== "complete" && (task.scheduled_date === today || task.due_date === today),
      )
      .map((task) => ({
        id: `task-${task.id}`,
        label: task.title,
  ...
  const upcoming = useMemo(() => {
    ...
    const fromEvents = (events.data ?? [])
      .filter(
        (event) =>
          event.start_date >= today && event.start_date <= by && event.status !== "cancelled",
      )
      .map((event) => ({
        id: `event-${event.id}`,
  ...
  const alerts = useMemo(() => {
    ...
    const overdueTasks = (tasks.data ?? [])
      .filter(
        (task) =>
          task.status !== "complete" && Boolean(task.due_date) && (task.due_date as string) < today,
      )
      .map((task) => ({
        id: `task-${task.id}`,
        label: task.title,
  ...
  return (
    <>
      <PageHeader
        title="Dashboard"
        description="Everything in motion right now, with the money attached."
      />
      ...
      <StatGrid>
        ...
        <StatCell
          label="Income"
          value={formatMoney(totalIncome(txns))}
          caption="All recorded income"
        />
        <StatCell
          label="Expense"
          value={formatMoney(totalExpense(txns))}
          caption="All recorded spend"
        />
        <StatCell
          label="Net cash flow"
          value={formatMoney(netCashFlow(txns))}

...
export const Route = createFileRoute("/_authenticated/tasks")({
    head: () => ({
        meta: [
            ...
            {
                name: "description",
                content: "Track every task by status, priority and due date, linked to the event it serves.",
            },
...
function TasksPage() {
    ...
    const eventName = (id: string | null) => (events.data ?? []).find((event) => event.id === id)?.name ?? "-";
    ...
    const columns: ReadonlyArray<Column<Row<"tasks">>> = [
        ...
        { key: "due", header: "Due", cell: (task) => formatDate(task.due_date) },
        {
            key: "priority",
            header: "Priority",
            cell: (task) => <StatusBadge tone={priorityTone[task.priority] ?? "neutral"}>{task.priority}</StatusBadge>,
        },
        {
            key: "actions",
            header: "",

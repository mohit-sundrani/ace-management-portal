...
function StatementsPage() {
    ...
    const opening = account
        ? toMinor(account.opening_balance) +
          netCashFlow(
              txns.filter((txn) => txn.account_id === account.id && (!startDate || txn.occurred_on < startDate)),
          )
        : 0;
    ...
    const columns: ReadonlyArray<Column<StatementRow>> = [
        ...
        { key: "description", header: "Description", cell: (row) => row.description },
        {
            ...
            header: "In",
            align: "right",
            cell: (row) => (row.inflow ? <span className="text-success">{formatMoney(row.inflow)}</span> : "-"),
        },
        {
            key: "outflow",
            header: "Out",

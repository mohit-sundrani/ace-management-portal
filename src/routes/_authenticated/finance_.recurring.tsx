    ...
    return (
        <>
            ...
            <StatGrid className="xl:grid-cols-3">
                <StatCell label="Active rules" value={active.length} caption="Running on schedule" emphasis />
                <StatCell label="Paused" value={rows.length - active.length} caption="Kept for later" />
                <StatCell label="Next run" value={nextRun ? formatDate(nextRun) : "-"} caption="Earliest active" />
            </StatGrid>
            ...
            <RecordDialog
                ...
                onSubmit={(values) => create.mutate(payload(values) as never, { onSuccess: () => setOpen(false) })}
            />

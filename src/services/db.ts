...
export const friendly = (action: string, code?: string): string => {
    if (code === "42501" || code === "PGRST301") return "You don't have permission to perform this action.";
...
const table_ = (name: TableName): Builder => (supabase as unknown as { from: (n: string) => Builder }).from(name);
...
export async function listRows<T extends TableName>(name: T, options: ListOptions = {}): Promise<Array<Row<T>>> {
...
export async function insertRow<T extends TableName>(name: T, values: InsertRow<T>): Promise<Row<T>> {
...
export async function updateRow<T extends TableName>(name: T, id: string, values: UpdateRow<T>): Promise<Row<T>> {

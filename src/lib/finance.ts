...
export const totalIncome = (txns: ReadonlyArray<Txn>): Minor => sumMinor(txns.filter(isIncome).map((t) => t.amount));

export const totalExpense = (txns: ReadonlyArray<Txn>): Minor =>
    sumMinor(txns.filter((t) => !isIncome(t)).map((t) => t.amount));

export const netCashFlow = (txns: ReadonlyArray<Txn>): Minor => totalIncome(txns) - totalExpense(txns);
...
export const eventFinance = (
    ...
    const plannedIncome = sumMinor(scopedItems.filter((i) => i.kind === "income").map((i) => i.planned_amount));
    const plannedExpenseItems = sumMinor(scopedItems.filter((i) => i.kind === "expense").map((i) => i.planned_amount));
...
export const withRunningBalance = (txns: ReadonlyArray<Txn>, opening: Minor): Array<Txn & { balance: Minor }> => {
    let balance = opening;
    return [...txns]
